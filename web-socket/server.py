...
ESP_IP = os.environ.get("ESP_IP", "192.168.0.159")  # Defaults to ESP IP, change as needed
...
def calculate_sensor_simulation(dt: float):
    ...
    if relay_states["lights"] == 1:
        current += 0.25
    if relay_states["lights2"] == 1:
        current += 0.25
    if relay_states["lights3"] == 1:
        current += 0.25
...
async def websocket_esp(websocket: WebSocket):
    global esp_connection, esp_connected_status
    ...
    try:
        async for message in websocket.iter_text():
            ...
            # Parse ESP response e.g. "Relay1 ON"
            device = None
            state_val = None
...
async def get_index():
    ...
    return HTMLResponse("index.html not found on server.", status_code=404)
                ...
                if msg.get("type") == "command":
                    ...
                    if device in relay_states:
                        ...
                        if esp_connected_status and esp_connection:
                            # Map device and state to ESP command string
                            cmd_str = ""
                            if device == "lights":
